...
Parser.prototype.log_error = function(err, source){
  //return console.log('Fuck it up');
  var path = err && err.stack && err.stack.split && err.stack.split('\n')[1];
  console.error('json '+JSON.stringify({
        error:{
          message: err.message,
          name: err.name,
          stack: path ? path.trim() : err.stack
        },
        source: source,
        message: 'Email fault'
      }));
...
Parser.prototype._tryread = function(n) {
  //we do not need odmains here - try/catch will be enough
  //var dom  = domain.create();

  //dom.on('error', function(err){
  //  this.log_error(err, 'parser_domain');
  //}.bind(this));
  //
  //dom.run(function(){
  //  try {
    ...
    //}catch(err){
    //  this.log_error(err, 'parser_catch');
    //}
  //}.bind(this));
};

Parser.prototype._parse = function(data) {
  this.debug && this.debug('>< ' + data.toString().trim());
  ...
  if (this._literallen > 0) {
    if (this._body) {
      ...
      if (datalen >= this._literallen) {
        ...
        if (datalen > litlen) {
            this.debug && this.debug('<= ' + inspect(data.slice(0, litlen).toString()));
            body.push(data.slice(0, litlen));
        }else {
            this.debug && this.debug('<= ' + inspect(data.toString()));
            body.push(data);
        }
        body.push(null);
      } else {
  ...
  while (i < datalen) {
    ...
    if (idxlf === -1) {
      ...
      !this._body && this.debug && this.debug('<= ' + inspect(this._buffer));

      if (RE_PRECEDING.test(this._buffer)) {
        var firstChar = this._buffer[0];
        if (firstChar === '*') {
          //here we become ready for broken first line
          //needed for cases when the answer is the email body
          //if((i < data.length) && (data[i] == 32)) continue;
          this._resUntagged();
        }else if (firstChar === 'A')
        //}else if (firstChar === 'C')//163.com
          this._resTagged();
        else if (firstChar === '+')
          this._resContinue();
...
function parseFetch(text, literals, seqno) {
  ...
  for (var i = 0, len = list.length, key, val; i < len; i += 2) {
    //SIEGE
    //null.toString();
    key = list[i] && list[i].toString().toLowerCase();
    val = list[i + 1];
    if (key === 'envelope')
...
function parseBodyStructure(cur, literals, prefix, partID) {
  ...
  if (prefix === undefined) {
    ...
    if (Array.isArray(cur[0])) { // multipart
      ...
      //SIEGE 2
      //part=6;part = part.toLowerCase();
      //LATE SIEGE too:
      //undefined.toString()
      part = { type: (cur[next++] || '').toString().toLowerCase() };
      if (partLen > next) {
      ...
      if (typeof cur[1] === 'string') {
        part = {
          ...
          //SIEGE 3
          type: cur[0] && cur[0].toLowerCase() || '', subtype: cur[1].toLowerCase(),
        ...
        //SIEGE
        //null.toLowerCase();
        part = { type: cur[0] && cur[0].toLowerCase() || '', params: null };
        cur.splice(1, 0, null);
        ++partLen;
        next = 2;
      }
      if (Array.isArray(cur[2])) {
        ...
        for (i = 0, len = cur[2].length; i < len; i += 2){
          ...
          if(cur[2][i])
            //SIEGE - undefined is not a function   
            part.params[cur[2][i].toString().toLowerCase()] = cur[2][i + 1];
        }
...
function parseExpr(o, literals, result, start, useBrackets) {
  ...
  //instead of UNCANCELLED SIEGE
  //Cannot read property 'str' of null|undefined
  if(!o) o = {};
  if(!o.str) o.str = '';
...
//VERY LATE SIEGE
  ...
  for (i = state.replaces.length - 1; i >= 0; --i) {
    ...
    if (Array.isArray(seq)) {
      ...
      //VERY LATE SIEGE
      //Parser.js:1029 str = str.substring(0, seq.fromOffset) ^ TypeError: undefined is not a function
