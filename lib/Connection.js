...
Connection.prototype.connect = function() {
  ...
  this._tagcount = 0;
  //TODO: why? no clearTimeout. no checks. do not re-connect, initialize new instance.
  this._tmrConn = undefined;
  this._tmrKeepalive = undefined;
  this._tmrAuth = undefined;
  this._queue = [];
  ...
  if (config.tls){
    ...
    allemit(this._sock, 'tls_socket', self.debug);
  }else {
    socket.once('connect', onconnect);
    this._sock = socket;
  }
  allemit(socket, 'plain_socket', self.debug);
  ...
  function onconnect() {
    ...
    self._tmrAuth = setTimeout(function() {
      if(self && self.listeners && self.listeners('error') && self.listeners('error').length > 0){
        var err = new Error('Timed out while authenticating with server for ' + config.email);
  ...
  var socket_on_close = function (had_err) {
  ...
  socket.on('close', socket_on_close);

  //SIEGE 31.late
  if(config.tls)
    this._sock.on('close', socket_on_close);
  ...
  //SIEGE 31.very.very.late
  if(config.tls)
    this._sock.once('end', this._onSocketEnd);

  this._parser = parser = new Parser(this._sock, this.debug);

  parser.on('untagged', self._resUntagged.bind(self));
  parser.on('tagged', self._resTagged.bind(self));
  parser.on('body', function(stream, info) {
    ...
    if (msg === undefined) {
      ...
      allemit(msg.msgEmitter, 'msg_emitter', this.debug);
  ...
  parser.on('continue', function(info) {
    //SIEGE 31
    if(self._fuckUp('parseContinue')) return;
    self.emit('alive');
...
Connection.prototype.openBox = function(name, readOnly, cb) {
  ...
  this._enqueue(cmd, function(err) {
    if (err) {
      ...
      if(!self._box) self._createCurrentBox();
      self._box.name = name;
      cb(err, self._box);
    }
  });
...
Connection.prototype._fetch = function(which, uids, options) {
  ...
  req.bodyEmitter = new EventEmitter();
  allemit(req.bodyEmitter, 'body_emitter', this.debug);

  return req.bodyEmitter;
};
...
Connection.prototype._resUntagged = function(info) {
  ...
  if(this._curReq) this.emit('alive');
  ...
  if (type === 'bye')
    ...
    if(this._fuckUp('_resUntagged')) return;
    if (/^(?:UID )?FETCH/.test(this._curReq.fullcmd)) {
      ...
      if (msg === undefined) {
        ...
        msgEmitter = new EventEmitter();
        allemit(msgEmitter, 'msg_emitter', this.debug);
...
Connection.prototype._fuckUp = function(place){
  if(!this._curReq){
    var err = new Error('no _curReq in connection '+ place);
    err.source = 'imap-curreq';
    this.emit('error', err);
    this.debug && this.debug('-- siege no _curReq ' + place);
    this._sock.end();
    return true;
  }
  return false;
...
Connection.prototype._resTagged = function(info) {
  if(this._fuckUp('_resTagged')) return;
  this.emit('alive');
...
Connection.prototype._doKeepaliveTimer = function(immediate) {
  this.debug && this.debug('-- _doKeepaliveTimer ' + (immediate || '').toString());
  var self = this,
      ...
      timerfn = function() {
        ...
        if (self._idle.enabled) {
          ...
          if (!self.serverSupports('IDLE')
            ...
            //console.log('ALIVE - setTimeout(timerfn, interval)');
...
Connection.prototype._login = function(without_starttls) {
...
Connection.prototype._starttls = function() {
  ...
  this._enqueue('STARTTLS', function(err) {
    ...
    self._sock.once('end', self._onSocketEnd);
    ...
    allemit(self._sock, 'startls_socket', self.debug);

    self._sock.on('error', self._onError);
    self._sock.on('timeout', self._onSocketTimeout);
    self._sock.once('end', self._onSocketEnd);

    self._sock.setTimeout(this._config.socketTimeout);
...
function escape(str) {
  //SIEGE 31 - REALY CANCELED - bad place to protect, take a look deeper
