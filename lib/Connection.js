...
Connection.prototype.connect = function() {
  ...
  this._onSocketClose = function (had_err) {
    clearTimeout(self._tmrConn);
    clearTimeout(self._tmrAuth);
    clearTimeout(self._tmrKeepalive);
    self.state = 'disconnected';
  ...
  //SIEGE 31.late
  if(config.tls)
    this._sock.once('close', this._onSocketClose);
...
Connection.prototype.openBox = function(name, readOnly, cb) {
  ...
  this._enqueue(cmd, function(err) {
    if (err) {
      ...
      if(!self._box){
        self._createCurrentBox();
        self.notifyEdited('openBox');
      }
      self._box.name = name;
      cb(err, self._box);
    }
  ...
  if (type === 'bye')
    ...
    if (this._box) {
      ...
      if (now > prev && this.state === 'authenticated') {
        ...
        for(var x = now, seqs = []; x > prev; seqs.push(x--));
        this.emit('mail', seqs);
      }else{
        this.emit('exists', now);
      }
    }
...
Connection.prototype._starttls = function() {
  ...
  this._enqueue('STARTTLS', function(err) {
    ...
    self._sock.once('close', self._onSocketClose);
